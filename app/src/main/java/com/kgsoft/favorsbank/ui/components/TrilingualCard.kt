...
import com.kgsoft.favorsbank.data.TrilingualZikr
...
/**
 * Three-line zikr display:
 *  1. the fixed Arabic original (always first, always present),
 *  2. the Latin transliteration (pronunciation guide),
 *  3. the meaning in the current UI language (hidden when the UI is Arabic).
 *
 ...
 */
@Composable
fun TrilingualCard(tri: TrilingualZikr, modifier: Modifier = Modifier) {
    Card(
        modifier = modifier.fillMaxWidth(),
        colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface),
        elevation = CardDefaults.cardElevation(defaultElevation = 2.dp)
    ) {
        TrilingualContent(tri, modifier = Modifier.padding(16.dp))
    }
}
 ...
 */
@Composable
fun TrilingualContent(tri: TrilingualZikr, modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        verticalArrangement = Arrangement.spacedBy(10.dp)
            ...
            // 2 — transliteration
            if (tri.transliteration.isNotBlank()) {
                Text(
                    text = tri.transliteration,
            ...
            // 3 — meaning in the user's language
            if (lang != "ar") {
                Text(
                    text = tri.meaning(lang),
                    fontSize = 15.sp,
                    lineHeight = 26.sp,
                    color = GreenPrimary,
                    modifier = Modifier.fillMaxWidth()
